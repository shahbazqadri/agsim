...
class AICO_solver(object):
    def __init__(self, model, xo, xf, disturbance = 'nodist'):
        ...
        if self.model == 'linear':
            ...
            self.omega_limit = [-0.5, -1.5] # angular velocity limits for the unicycle model
            self.h   = 0.001
            self.H_u = 1e-3*np.eye(self.un)
            self.H_upperbound = 1e-3*np.eye(self.un)
            self.H_lowerbound = 1e-3*np.eye(self.un)
            self.H = self.H_u + self.H_upperbound + self.H_lowerbound
            umax = -0.5
            umin = -1.5
            self.hu = np.dot(umax, self.H_upperbound) + np.dot(umin, self.H_lowerbound)
    ...
    def get_system_dynamics(self,xhat):
        ...
        # Unicycle model
            ...
            self.Q = self.h * compute_covariance(self, xhat)
    ...
    def compute_forward_message(self):
        ...
        s = ap + np.dot(Ap, np.dot(Sbar,(np.dot(Sinvp,sp)+rp))) + np.dot(np.dot(Bp,np.linalg.inv(H)),self.hu)
    ...
    def compute_backward_message(self):
        ...
        v = -np.dot(np.linalg.inv(Ac), (ac +np.dot(np.dot(Bc,np.linalg.inv(H)),self.hu))) + np.dot(np.linalg.inv(Ac),np.dot(Vbar,(np.dot(Vinvn,vn)+rn)))
        V = np.dot(np.linalg.inv(Ac), np.dot(W+Vbar, np.linalg.inv(Ac.transpose())))
    ...
    def compute_feedback_controller(self):
       ...
       such that, o_t &= M_t^{-1}\bigg( B_t^T (Q_t + \bar{V}_{t+1}^{-1})^{-1}\bar{V}_{t+1}^{-1}\bar{\nu}_{t+1}  - B_t^T (Q_t + \bar{V}_{t+1}^{-1})^{-1} a_t \bigg)\\
        ...
        o2  = np.dot(Bc.transpose(), np.dot(Vstar, ac+ np.dot(np.dot(Bc, np.linalg.inv(H)),self.hu)))
    ...
    def plot(self):
            if self.model == 'unicycle':
                ...
                plt.plot(self.X[1,:]-self.Xu[1,:], label = 'errorx', color= 'red', linewidth = '2')
                plt.legend()
                plt.title('x0 = [{x01}, {x02}, {x03}, {x04}] m, xg = [{xg1}, {xg2}] m,\n velocity = {v} m/s, Timesteps = {T}, timestep= {h} sec'.format(x01= self.x0_tolist[0], x02= self.x0_tolist[1], x03= self.x0_tolist[2], x04= self.x0_tolist[3],  xg1= self.xT[0], xg2= self.xT[1], v= self.vel, T= self.T, h= self.h))
                plt.show()
...
xo = np.array([[-100],[20],[3.14],[100]])
xf = [-10, 100]
disturbance = 'nodist'
sol1 = AICO_solver('unicycle', xo, xf, disturbance)
print('running AICO')
X_withoutdist = sol1.AICO()
disturbance = 'withdist'
sol2 = AICO_solver('unicycle', xo, xf, disturbance)
print('running AICO')
...
plt.plot(X_withdist[0,:], X_withdist[1,:], label = 'xy-trajectory with disturbance', color= 'blue', linewidth = '2')
plt.legend()
